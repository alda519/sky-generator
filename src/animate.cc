...
int main(int argc, char **argv)
  {
    ...
    // vytvoreni okna
    SDL_Window *screen = SDL_CreateWindow(argv[0], SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, buffer.width, buffer.height, 0);
    if(screen == NULL) return 1;
    SDL_Renderer *renderer = SDL_CreateRenderer(screen, -1, 0);
    if(renderer == NULL) return 1;

    // vyplneni okca cernou
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
    SDL_RenderClear(renderer);
    SDL_RenderPresent(renderer);

    // vytvoreni textury
    SDL_Texture *sdlTexture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB888, SDL_TEXTUREACCESS_STREAMING, buffer.width, buffer.height);
    if(sdlTexture == NULL) return 1;
    ...
    for (i = 0; i < params.frames; i++)
      {
        ...
        // kopie bufferu do Uint32 pole, ktere je potreba
        // mozna by se mohl primo color_buffer udelat 4B/pixel
        Uint32 *myPixels = new Uint32[buffer.width * buffer.height];
        for(unsigned int i = 0; i < buffer.height; ++i)
            for(unsigned int j = 0; j < buffer.width; ++j) {
                unsigned char *ptr = & buffer.data[3*i*buffer.width + 3*j];
                myPixels[i*buffer.width + j] = (ptr[0] << 16) + (ptr[1] << 8) + (ptr[2] << 0);
            }

        // kopie sceny do textury
        SDL_UpdateTexture(sdlTexture, NULL, myPixels, buffer.width*sizeof(Uint32));
        delete myPixels;

        // zobrazit na obrazovku
